...
```{r}
# Alternativa:
boxplot.stats(nba.unique$Weight)$out
```
...
# METODO ENRIQUE: top50_jugadores_bajos <- nba.unique[order(nba.unique$Height)[1:50],6] # creo un vector con los jugadores más bajos usando order de la varibale height y que guarde la columna 6 que es el nombre de los jugadores.
...
# Pedo ordernar por Height y luego pedir las primeras 50 columnas
nba.avg %>%
  arrange(desc(Height)) %>%
  slice_head(n = 50)
...
arrange(nba.avg$Altura[1:50])

Mas_altos <- head(arrange(nba.avg, desc(head), n = 50)
arrange(nba.avg, desc(head)
...
head(arrange(nba.avg, desc(Weight)), n = 50) # Tendriamos los 50 mas pesados
tail(arrange(nba.avg, desc(Weight)), n = 50) 
